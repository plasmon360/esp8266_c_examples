/*Client sends GPIO4 input to plotly once in a while with a timestamp. Device will go to deep sleep between wakeups. 
Notes
Timing:
...
MAKE SURE XPD is connected to RST, otherwise it wont wake up from deep sleep
HI of phototransistor is connected to 3.3v and other side to 10Kohm resistor which connects to ground. Voltage at phototransistor and resistor junction is 3.3 V if there is light and less than 3.3 V if there is no light.
...
sint16 sleep_delay_time_sec = 10;
...
void ICACHE_FLASH_ATTR connectCB(void *arg)
{
    ...
    if (current_time_with_dst !=NULL && stove_on_or_off == 1)
	{
		...
		os_printf("Attempting to send %s, ON\n", date_string);
		os_sprintf(text_data,"un=%s&key=%s&origin=plot&platform=esp8266_c&args=[{\"x\":\"%s\", \"y\":\"ON\"}]&kwargs={\"filename\": \"My Stove status from ESP-8266\",\"fileopt\": \"extend\",\"style\": {\"type\": \"scatter\", \"mode\": \"markers\", \"marker\": {\"symbol\": \"circle\", \"color\": \"purple\",\"size\": \"12\"}},\"layout\":{\"title\": \"My Stove status\", \"xaxis\": {\"name\": \"Date Time\",\"title\": \"Date Time\"}, \"yaxis\": {\"name\": \"ON/OFF\",\"title\": \"ON/OFF\"}}}",plotly_username, plotly_key, date_string);
	    os_sprintf(send_data_buffer, "POST %s HTTP/1.1\r\nHost: %s\r\nConnection: close\r\nContent-Length: %d\r\n\r\n%s", plotly_rest_api_home, plotly_host, strlen(text_data), text_data); // SEnd data and close the connection.
	    os_printf("Data that was sent is %s\n\r",send_data_buffer);
	  espconn_send(pNewEspConn, send_data_buffer, sizeof(send_data_buffer));
        } 
   else
	{
	os_printf("Not sending stove status, because status is off\n");
	espconn_disconnect(pNewEspConn);
	}
}
...
{
    os_timer_disarm(&sleeptimer); // disarm any timer
    os_timer_setfn(&sleeptimer, (os_timer_func_t *)go_to_sleep, NULL);	// associate a callback for a timer
    os_timer_arm(&sleeptimer, sleep_delay_time_sec*1000, 0); // Wait some time to post request to plotly and then go to sleep. Will sleep irrespective of response or not.
